...
var selector = casper.cli.options.selector || 'form'
var dest = casper.cli.options.dest || './results/default/phantomjs_720'
var width = parseInt(casper.cli.options.width) || 720
var hover = ''
...
if (verbose) {
  casper.echo('loading: ' + url + ', selector: "' + selector +
    '", saving "' + dest + '.*"', 'INFO')
}
try {
  ...
  var getElementInfo = function (element) {
    ...
    if (document.defaultView.getComputedStyle(element, ':before').getPropertyValue('content')) {
      children.push({
        ...
        textContent: document.defaultView.getComputedStyle(element, ':before')
    ...
    if (document.defaultView.getComputedStyle(element, ':after').getPropertyValue('content')) {
      children.push({
        ...
        textContent: document.defaultView.getComputedStyle(element, ':after')
          .getPropertyValue('content'),
        elementId: '',
        cssclass: '',
        type: '',
...
casper.open(url, function () {
  ...
  .then(function () {
    ...
    if (selectorFound) {
      ...
      if (verbose) {
        this.echo(dest + '.css.json' + ' saved', 'INFO')
      }
      if (selector.indexOf('/') === 0) {
      ...
      if (verbose) {
        this.echo(dest + '.png' + ' saved', 'INFO')
      }
    } else {
      this.echo('element not found: "' + selector + '"', 'ERROR')
    ...
    if (hasScrollbar === true) {
      ...
      fs.write(destDir + '/result.log', url + ' ' + selector +
               ' page has horizontal scrollbar\n', 'a')
    } else {
